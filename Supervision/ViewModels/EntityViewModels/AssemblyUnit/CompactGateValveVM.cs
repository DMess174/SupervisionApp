...
{
    public class CompactGateValveVM : BasePropertyChanged
    {
            ...
            get
            {
                return editItem ?? (
                    editItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        {
                            var wn = new CompactGateValveEditView();
                            var vm = new CompactGateValveEditVM(SelectedItem.Id, SelectedItem);
                            wn.DataContext = vm;
                            w?.Close();
                            wn.ShowDialog();
                        }
                ...
                return copyItem ?? (
                    copyItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            db.Set<CompactGateValve>().Add(item);
                            db.SaveChanges();
                            var Journal = db.Set<CompactGateValveJournal>().Where(i => i.DetailId == SelectedItem.Id).ToList();
                            var coatingJournal = db.Set<CoatingJournal>().Where(i => i.DetailId == SelectedItem.Id).ToList();
                            foreach (var record in Journal)
                            {
                                var Record = new CompactGateValveJournal()
                                {
                                    Date = record.Date,
                                    DetailId = item.Id,
                                ...
                                db.Set<CompactGateValveJournal>().Add(Record);
                                db.SaveChanges();
                            }
            ...
            get
            {
                return addItem ?? (
                    addItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        db.SaveChanges();
                        SelectedItem = item;
                        var tcpPoints = db.Set<CompactGateValveTCP>().ToList();
                        var coatingPoints = db.Set<CoatingTCP>().ToList();
                        foreach (var i in tcpPoints)
                        {
                            var journal = new CompactGateValveJournal()
                            {
                            ...
                            if (journal != null)
                            {
                                db.Set<CompactGateValveJournal>().Add(journal);
                                db.SaveChanges();
                            }
                        }
            ...
            get
            {
                return removeItem ?? (
                    removeItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            db.Set<CompactGateValve>().Remove(SelectedItem);
                            db.SaveChanges();
                        }
                        else MessageBox.Show("Объект не выбран!", "Ошибка");
                    }));
        ...
        public CompactGateValveVM()
        {
            db = new DataContext();
            db.Set<CompactGateValve>().Load();
            AllInstances = db.Set<CompactGateValve>().Local.ToObservableCollection();
