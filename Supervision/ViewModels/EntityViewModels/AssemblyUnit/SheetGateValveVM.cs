...
{
    public class SheetGateValveVM : BasePropertyChanged
    {
            ...
            get
            {
                return editItem ?? (
                    editItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        {
                            var wn = new SheetGateValveEditView();
                            var vm = new SheetGateValveEditVM(SelectedItem.Id, SelectedItem);
                            wn.DataContext = vm;
                            w?.Close();
                            wn.ShowDialog();
                        }
                ...
                return copyItem ?? (
                    copyItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            db.Set<SheetGateValve>().Add(item);
                            db.SaveChanges();
                            var Journal = db.Set<SheetGateValveJournal>().Where(i => i.DetailId == SelectedItem.Id).ToList();
                            var coatingJournal = db.Set<CoatingJournal>().Where(i => i.DetailId == SelectedItem.Id).ToList();
                            foreach (var record in Journal)
                            {
                                var Record = new SheetGateValveJournal()
                                {
                                    Date = record.Date,
                                    DetailId = item.Id,
                                ...
                                db.Set<SheetGateValveJournal>().Add(Record);
                                db.SaveChanges();
                            }
            ...
            get
            {
                return addItem ?? (
                    addItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        db.SaveChanges();
                        SelectedItem = item;
                        var tcpPoints = db.Set<SheetGateValveTCP>().ToList();
                        var coatingPoints = db.Set<CoatingTCP>().ToList();
                        foreach (var i in tcpPoints)
                        {
                            var journal = new SheetGateValveJournal()
                            {
                            ...
                            if (journal != null)
                            {
                                db.Set<SheetGateValveJournal>().Add(journal);
                                db.SaveChanges();
                            }
                        }
            ...
            get
            {
                return removeItem ?? (
                    removeItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            db.Set<SheetGateValve>().Remove(SelectedItem);
                            db.SaveChanges();
                        }
                        else MessageBox.Show("Объект не выбран!", "Ошибка");
                    }));
        ...
        public SheetGateValveVM()
        {
            db = new DataContext();
            db.Set<SheetGateValve>().Load();
            AllInstances = db.Set<SheetGateValve>().Local.ToObservableCollection();
