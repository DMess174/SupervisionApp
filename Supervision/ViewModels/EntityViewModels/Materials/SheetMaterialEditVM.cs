...
namespace Supervision.ViewModels.EntityViewModels.Materials
{
    public class SheetMaterialEditVM : BasePropertyChanged
    {
        ...
        public SheetMaterialEditVM(int id, BaseTable entity)
        {
            ...
            FirstSizes = db.SheetMaterials.Select(t => t.FirstSize).Distinct().OrderBy(x => x).ToList();
            SecondSizes = db.SheetMaterials.Select(t => t.SecondSize).Distinct().OrderBy(x => x).ToList();
            ThirdSizes = db.SheetMaterials.Select(d => d.ThirdSize).Distinct().OrderBy(x => x).ToList();
