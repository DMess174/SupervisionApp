...
{
    public class WeldingMaterialVM<TEntity, TEntityTCP, TEntityJournal> : BasePropertyChanged
            ...
            get
            {
                return editItem ?? (
                    editItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        {
                            var wn = new WeldingMaterialEditView();
                            var vm = new WeldingMaterialEditVM<TEntity, TEntityTCP, TEntityJournal>(SelectedItem.Id, SelectedItem);
                            wn.DataContext = vm;
                            w?.Close();
                            wn.ShowDialog();
                        }
                        ...
                        if (SelectedItem != null)
                        {
                            ...
                            foreach (var record in Journal)
                            {
                                var Record = new TEntityJournal()
                                {
                                    ...
                                    DetailNumber = item.Name,
                                    DetailDrawing = item.Batch,
