...
{
    public class SlamShutterVM : ViewModelBase
    {
        ...
        public static SlamShutterVM LoadVM(DataContext context)
        {
            SlamShutterVM vm = new SlamShutterVM(context);
        ...
        private async Task UpdateList()
        {
            ...
            {
                IsBusy = true;
                AllInstances = new ObservableCollection<SlamShutter>();
        ...
        private async Task AddNewItem()
        {
            ...
            {
                IsBusy = true;
                SelectedItem = await repo.AddAsync(new SlamShutter());
                var tcpPoints = await repo.GetTCPsAsync();
                var records = new List<SlamShutterJournal>();
                foreach (var tcp in tcpPoints)
                {
                    var journal = new SlamShutterJournal(SelectedItem, tcp);
                    if (journal != null)
                        records.Add(journal);
                }
        ...
        private async Task CopySelectedItem()
        {
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    var copy = await repo.AddAsync(new SlamShutter(temp));
                    var jour = new ObservableCollection<SlamShutterJournal>();
                    foreach (var i in temp.SlamShutterJournals)
                    {
                        var record = new SlamShutterJournal(copy.Id, i);
                        jour.Add(record);
                    }
        ...
        private void EditSelectedItem()
        {
            if (SelectedItem != null)
            {
                ...
                {
                    DataContext = SlamShutterEditVM.LoadVM(SelectedItem.Id, SelectedItem, db)
                };
            }
        ...
        public SlamShutterVM(DataContext context)
        {
            db = context;
