...
{
    public class SaddleVM : BasePropertyChanged
    {
        private readonly DataContext db;
        private IEnumerable<Saddle> allInstances;
                ...
                return editItem ?? (
                    editItem = new DelegateCommand<Window>((w) =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            var vm = new SaddleEditVM(SelectedItem.Id, SelectedItem);
                            wn.DataContext = vm;
                            w?.Close();
                            wn.ShowDialog();
                        }
                ...
                return copyItem ?? (
                    copyItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            db.Saddles.Add(item);
                            db.SaveChanges();
                            var Journal = db.SaddleJournals.Where(i => i.DetailId == SelectedItem.Id).ToList();
                            foreach (var record in Journal)
                            {
                                var Record = new SaddleJournal()
                                {
                                    Date = record.Date,
                                    DetailId = item.Id,
            ...
            get
            {
                return addItem ?? (
                    addItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        db.SaveChanges();
                        SelectedItem = item;
                        var tcpPoints = db.SaddleTCPs.ToList();
                        foreach (var i in tcpPoints)
                        {
                            var journal = new SaddleJournal()
                            {
            ...
            get
            {
                return removeItem ?? (
                    removeItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            db.Saddles.Remove(SelectedItem);
                            db.SaveChanges();
                        }
                        else MessageBox.Show("Объект не выбран!", "Ошибка");
                    }));
        ...
        public IEnumerable<Saddle> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        {
            db = new DataContext();
            db.Saddles.Include(i => i.MetalMaterial).Load();
            AllInstances = db.Saddles.Local.ToObservableCollection();
