...
{
    public class FrontalSaddleSealingVM : BasePropertyChanged
    {
                ...
                return editItem ?? (
                    editItem = new DelegateCommand<Window>((w) =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            var vm = new FrontalSaddleSealingEditVM(SelectedItem.Id, SelectedItem);
                            wn.DataContext = vm;
                            w?.Close();
                            wn.ShowDialog();
                        }
            ...
            get
            {
                return copyItem ?? (
                    copyItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            var item = new FrontalSaddleSealing()
                            {
                            ...
                            db.FrontalSaddleSeals.Add(item);
                            db.SaveChanges();
                            var Journal = db.FrontalSaddleSealingJournals.Where(i => i.DetailId == SelectedItem.Id).ToList();
                            foreach (var record in Journal)
                            {
                                var Record = new FrontalSaddleSealingJournal()
                                {
                                    Date = record.Date,
                                    DetailId = item.Id,
                                ...
                                db.FrontalSaddleSealingJournals.Add(Record);
                                db.SaveChanges();
                            }
            ...
            get
            {
                return addItem ?? (
                    addItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        db.SaveChanges();
                        SelectedItem = item;
                        var tcpPoints = db.FrontalSaddleSealingTCPs.ToList();
                        foreach (var i in tcpPoints)
                        {
                            var journal = new FrontalSaddleSealingJournal()
                            {
                            ...
                            if (journal != null)
                            {
                                db.FrontalSaddleSealingJournals.Add(journal);
                                db.SaveChanges();
                            }
                        }
                        var wn = new FrontalSaddleSealingEditView();
            ...
            get
            {
                return removeItem ?? (
                    removeItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            db.FrontalSaddleSeals.Remove(SelectedItem);
                            db.SaveChanges();
                        }
                        else MessageBox.Show("Объект не выбран!", "Ошибка");
                    }));
        ...
        public FrontalSaddleSealingVM()
        {
            db = new DataContext();
            db.FrontalSaddleSeals.Load();
            AllInstances = db.FrontalSaddleSeals.Local.ToObservableCollection();
