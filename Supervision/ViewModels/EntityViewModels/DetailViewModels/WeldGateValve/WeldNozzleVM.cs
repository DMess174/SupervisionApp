...
{
    public class WeldNozzleVM : BasePropertyChanged
    {
        private readonly DataContext db;
        private IEnumerable<WeldNozzle> allInstances;
        ...
        public string Certificate
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is WeldNozzle item && item.Certificate != null)
                    {
                ...
                return editItem ?? (
                    editItem = new DelegateCommand<Window>((w) =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            var vm = new WeldNozzleEditVM(SelectedItem.Id, SelectedItem);
                            wn.DataContext = vm;
                            w?.Close();
                            wn.ShowDialog();
                        }
                ...
                return copyItem ?? (
                    copyItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            db.WeldNozzles.Add(item);
                            db.SaveChanges();
                            var Journal = db.WeldNozzleJournals.Where(i => i.DetailId == SelectedItem.Id).ToList();
                            foreach (var record in Journal)
                            {
                                var Record = new WeldNozzleJournal()
                                {
                                    Date = record.Date,
                                    DetailId = item.Id,
                                ...
                                db.WeldNozzleJournals.Add(Record);
                                db.SaveChanges();
                            }
            ...
            get
            {
                return addItem ?? (
                    addItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        db.SaveChanges();
                        SelectedItem = item;
                        var tcpPoints = db.WeldNozzleTCPs.ToList();
                        foreach (var i in tcpPoints)
                        {
                            var journal = new WeldNozzleJournal()
                            {
                            ...
                            if (journal != null)
                            {
                                db.WeldNozzleJournals.Add(journal);
                                db.SaveChanges();
                            }
                        }
                        var wn = new WeldNozzleEditView();
            ...
            get
            {
                return removeItem ?? (
                    removeItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            db.WeldNozzles.Remove(SelectedItem);
                            db.SaveChanges();
                        }
                        else MessageBox.Show("Объект не выбран!", "Ошибка");
                    }));
        ...
        public IEnumerable<WeldNozzle> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        {
            db = new DataContext();
            db.WeldNozzles.Include(i => i.MetalMaterial).Load();
            AllInstances = db.WeldNozzles.Local.ToObservableCollection();
