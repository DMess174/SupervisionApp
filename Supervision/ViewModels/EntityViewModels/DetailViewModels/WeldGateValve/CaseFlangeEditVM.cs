...
{
    public class CaseFlangeEditVM: BasePropertyChanged
    {
        ...
        private IEnumerable<CaseFlangeTCP> points;
        private IEnumerable<Inspector> inspectors;
        private IEnumerable<CaseFlangeJournal> journal;
        private readonly BaseTable parentEntity;
        private CaseFlange selectedItem;
        private CaseFlangeTCP selectedTCPPoint;
        ...
        public IEnumerable<CaseFlangeJournal> Journal
        {
            get => journal;
            set
            {
        ...
        public IEnumerable<CaseFlangeTCP> Points
        {
            get => points;
            set
            {
            ...
            get
            {
                return saveItem ?? (
                    saveItem = new DelegateCommand(() =>
                    {
                        ...
                        {
                            db.CaseFlanges.Update(SelectedItem);
                            db.SaveChanges();
                            ...
                            db.CaseFlangeJournals.UpdateRange(Journal);
                            db.SaveChanges();
                        }
        ...
        public ICommand CloseWindow
        {
            get
            {
                return closeWindow ?? (
                    ...
                    {
                        if (parentEntity is CaseFlange)
                        {
                            var wn = new CaseFlangeView();
            ...
            get
            {
                return addOperation ?? (
                    addOperation = new DelegateCommand(() =>
                    {
                        ...
                        {
                            var item = new CaseFlangeJournal()
                            {
                            ...
                            db.CaseFlangeJournals.Add(item);
                            db.SaveChanges();
                            Journal = db.CaseFlangeJournals.Where(i => i.DetailId == SelectedItem.Id).OrderBy(x => x.PointId).ToList();
                        }
                    }));
            }
        }
        ...
        public CaseFlangeTCP SelectedTCPPoint
        {
        ...
        public CaseFlangeEditVM(int id, BaseTable entity)
        {
            parentEntity = entity;
            db = new DataContext();
            SelectedItem = db.CaseFlanges.Include(i => i.WeldGateValveCase).Include(i => i.MetalMaterial).SingleOrDefault(i => i.Id == id);
            Journal = db.CaseFlangeJournals.Where(i => i.DetailId == SelectedItem.Id).OrderBy(x => x.PointId).ToList();
            JournalNumbers = db.JournalNumbers.Where(i => i.IsClosed == false).Select(i => i.Number).Distinct().ToList();
            Drawings = db.CaseFlanges.Select(s => s.Drawing).Distinct().OrderBy(x => x).ToList();
            Materials = db.MetalMaterials.ToList();
            Inspectors = db.Inspectors.OrderBy(i => i.Name).ToList();
            Points = db.Set<CaseFlangeTCP>().ToList();
        }
    }
}
