...
{
    public class CoverFlangeVM : BasePropertyChanged
    {
        private readonly DataContext db;
        private IEnumerable<CoverFlange> allInstances;
        ...
        public string Material
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.MetalMaterial.Material != null)
                    {
        ...
        public string Melt
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.MetalMaterial.Melt != null)
                    {
        ...
        public string Certificate
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CoverFlange item && item.MetalMaterial.Certificate != null)
                    {
                ...
                return editItem ?? (
                    editItem = new DelegateCommand<Window>((w) =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            var vm = new CoverFlangeEditVM(SelectedItem.Id, SelectedItem);
                            wn.DataContext = vm;
                            w?.Close();
                            wn.ShowDialog();
                        }
                ...
                return copyItem ?? (
                    copyItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            ...
                            db.CoverFlanges.Add(item);
                            db.SaveChanges();
                            var Journal = db.CoverFlangeJournals.Where(i => i.DetailId == SelectedItem.Id).ToList();
                            foreach (var record in Journal)
                            {
                                var Record = new CoverFlangeJournal()
                                {
                                    Date = record.Date,
                                    DetailId = item.Id,
                                ...
                                db.CoverFlangeJournals.Add(Record);
                                db.SaveChanges();
                            }
            ...
            get
            {
                return addItem ?? (
                    addItem = new DelegateCommand<Window>((w) =>
                    {
                        ...
                        db.SaveChanges();
                        SelectedItem = item;
                        var tcpPoints = db.CoverFlangeTCPs.ToList();
                        foreach (var i in tcpPoints)
                        {
                            var journal = new CoverFlangeJournal()
                            {
                            ...
                            if (journal != null)
                            {
                                db.CoverFlangeJournals.Add(journal);
                                db.SaveChanges();
                            }
                        }
                        var wn = new CoverFlangeEditView();
            ...
            get
            {
                return removeItem ?? (
                    removeItem = new DelegateCommand(() =>
                    {
                        if (SelectedItem != null)
                        {
                            db.CoverFlanges.Remove(SelectedItem);
                            db.SaveChanges();
                        }
                        else MessageBox.Show("Объект не выбран!", "Ошибка");
                    }));
        ...
        public IEnumerable<CoverFlange> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        public CoverFlangeVM()
        {
            db = new DataContext();
            db.CoverFlanges.Include(i => i.MetalMaterial).Load();
            AllInstances = db.CoverFlanges.Local.ToObservableCollection();
