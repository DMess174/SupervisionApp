...
{
    public class CaseFlangeVM : ViewModelBase
    {
        private readonly DataContext db;
        private IList<CaseFlange> allInstances;
        ...
        public IList<CaseFlange> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        public static CaseFlangeVM LoadVM(DataContext context)
        {
            CaseFlangeVM vm = new CaseFlangeVM(context);
        ...
        private async Task UpdateList()
        {
            ...
            {
                IsBusy = true;
                AllInstances = new ObservableCollection<CaseFlange>();
        ...
        private async Task AddNewItem()
        {
            ...
            {
                IsBusy = true;
                SelectedItem = await repo.AddAsync(new CaseFlange());
                var tcpPoints = await repo.GetTCPsAsync();
                var records = new List<CaseFlangeJournal>();
                foreach (var tcp in tcpPoints)
                {
                    var journal = new CaseFlangeJournal(SelectedItem, tcp);
                    if (journal != null)
                        records.Add(journal);
                }
        ...
        private async Task CopySelectedItem()
        {
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    var copy = await repo.AddAsync(new CaseFlange(temp));
                    var jour = new ObservableCollection<CaseFlangeJournal>();
                    foreach (var i in temp.CaseFlangeJournals)
                    {
                        var record = new CaseFlangeJournal(copy.Id, i);
                        jour.Add(record);
                    }
        ...
        private void EditSelectedItem()
        {
            if (SelectedItem != null)
            {
                ...
                {
                    DataContext = CaseFlangeEditVM.LoadVM(SelectedItem.Id, SelectedItem, db)
                };
            }
        ...
        public CaseFlangeVM(DataContext context)
        {
            db = context;
