...
{
    public class CaseEdgeVM : ViewModelBase
    {
        private readonly DataContext db;
        private IList<CaseEdge> allInstances;
        ...
        public string Material
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CaseEdge item && item.MetalMaterial.Material != null)
                    {
        ...
        public string Melt
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CaseEdge item && item.MetalMaterial.Melt != null)
                    {
        ...
        public string Certificate
        {
            ...
            set
            {
                ...
                allInstancesView.Filter += (obj) =>
                {
                    if (obj is CaseEdge item && item.MetalMaterial.Certificate != null)
                    {
        ...
        public IList<CaseEdge> AllInstances
        {
            get => allInstances;
            set
            {
        ...
        public static CaseEdgeVM LoadVM(DataContext context)
        {
            CaseEdgeVM vm = new CaseEdgeVM(context);
        ...
        private async Task UpdateList()
        {
            ...
            {
                IsBusy = true;
                AllInstances = new ObservableCollection<CaseEdge>();
        ...
        private async Task AddNewItem()
        {
            ...
            {
                IsBusy = true;
                SelectedItem = await repo.AddAsync(new CaseEdge());
                var tcpPoints = await repo.GetTCPsAsync();
                var records = new List<CaseEdgeJournal>();
                foreach (var tcp in tcpPoints)
                {
                    var journal = new CaseEdgeJournal(SelectedItem, tcp);
                    if (journal != null)
                        records.Add(journal);
                }
        ...
        private async Task CopySelectedItem()
        {
            if (SelectedItem != null)
            {
                try
                {
                    ...
                    var copy = await repo.AddAsync(new CaseEdge(temp));
                    var jour = new ObservableCollection<CaseEdgeJournal>();
                    foreach (var i in temp.CaseEdgeJournals)
                    {
                        var record = new CaseEdgeJournal(copy.Id, i);
                        jour.Add(record);
                    }
        ...
        private void EditSelectedItem()
        {
            if (SelectedItem != null)
            {
                ...
                {
                    DataContext = CaseEdgeEditVM.LoadVM(SelectedItem.Id, SelectedItem, db)
                };
            }
        ...
        public CaseEdgeVM(DataContext context)
        {
            db = context;
